...
void i2s_parallel_setup(volatile i2s_dev_t *dev, const i2s_parallel_config_t *cfg) {
	...
	dev->sample_rate_conf.rx_bits_mod=cfg->bits; //Не имеет значения
	...
	*/
	...
	//Мы игнорируем возможность фракционного разделения здесь.
	...
	//Установите буфер, чтобы перейти к следующему (убедитесь, что здесь образуется петля)сь образуется петля)
